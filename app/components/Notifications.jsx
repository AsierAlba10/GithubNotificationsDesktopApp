...
const Notifications = ({ notifications, repositories, loading, onMarkAsRead }) => {
    ...
    // Añadimos una variable de estado para rastrear si es la primera carga
    ...
    useEffect(() => {
        ...
        // Cuando los datos se cargan, ya no estamos en carga inicial
        if (repoObjects.length > 0 || notifications.length > 0) {
    ...
    const renderRepositories = () => {
        ...
        // Si estamos en carga inicial, mostrar mensaje de carga
        if (initialLoad || (loading && allRepositories.length === 0)) {
            return (
                <>
                    <div className="empty-state loading-state">
                        <i className="fas fa-circle-notch fa-spin"></i>
                        <p>Cargando notificaciones...</p>
                        <p className="empty-state-subtitle">Estamos conectando con GitHub para obtener tus notificaciones.</p>
                    </div>
                </>
            );
        }

        // Si no hay notificaciones, mostrar mensaje normal
    ...
    // Usamos una variable solo para el conteo de notificaciones, no para reposWithNotifications
    const totalNotifications = notifications.length;

    // Si hay repositorios pero estamos cargando, mostramos una versión simplificada
    const reposWithNotificationsCount = allRepositories.filter(r => r.notifications.length > 0).length;
            ...
            <div className="notifications-toolbar">
                ...
                <div className="filter-controls">
                    ...
                    <button
                        ...
                        disabled={loading}
                    >
                        <i className="fas fa-bell"></i> Con notificaciones ({reposWithNotificationsCount})
                    </button>
                    <button
                        className={`filter-button ${showAllRepos ? 'active' : ''}`}
